...
| 维度 | VMWare 虚拟机 | Docker 容器 | 对比结果 |
|------|--------------|-------------|---------|
| **启动时间** | 42.46 秒 | 1.07 秒 | Docker **快 39.5倍** ⚡ |
| **CPU占用** | 3.35% | 3.03% | Docker 低 9.6% 💻 |
| **内存占用** | 30.1 MB | 32.6 MB | VM 少 8.3% 💾 |
| **磁盘占用** | 11.4 MB | 144.8 MB | VM 少 92.1% 💿 |
| **QPS** | 33,349 req/s | 26,579 req/s | VM 高 25.5% 🚀 |
| **响应延迟** | 29.99 ms | 37.62 ms | VM 快 25.4% ⏱️ |
...
- Docker 启动时间仅为 VM 的 **2.5%**（1.07s vs 42.46s），快 **39.5倍**
- 在弹性伸缩场景中，Docker 可以**秒级**响应流量变化
- **结论**：Docker 天然适合需要快速扩缩容的云原生应用

### 2️⃣ 资源占用：各有优势

- **内存占用**：VM 比 Docker 少 **8.3%**（30.1 MB vs 32.6 MB）
- **磁盘占用**：VM 比 Docker 少 **92.1%**（11.4 MB vs 144.8 MB）
- **CPU占用**：Docker 比 VM 低 **9.6%**（3.03% vs 3.35%）
- **结论**：VM 在存储占用方面优势明显，Docker 在 CPU 效率上略优
...
|------|-----------|---------------|------|
| **启动 10 个实例** | ~7 分钟（42.46s × 10） | ~11 秒（1.07s × 10） | Docker 快 **39.5 倍** |
...
- **QPS**：VM 达到 **33,349 req/s**，比 Docker 高 **25.5%**
- **响应延迟**：VM 平均延迟 **29.99 ms**，比 Docker 快 **25.4%**
- **传输速率**：VM 达到 **27,780 KB/s**，比 Docker 高 **25.5%**
- **结论**：本次测试中 VM 在网络性能方面表现更优，可能与网络配置有关
